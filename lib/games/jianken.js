module.exports = function (app, message, args) {
    return new Jianken(app, message, args);
}
...
class Jianken extends require('./gamebase') {
    ...
    onTrigger(reaction, user, isOn) {
        ...
        } else if (this.selections.has(user.id) && this.selections.get(user.id).emojiName === reaction.emoji.name)
    ...
    getEmojiArray() {
        ...
        EMOJIS.SPECIAL.forEach((name) => {
            const item = message.guild.emojis.find(emoji => emoji.name === name);
            if (item)
                ary.push(item);
        });
        return ary;
    ...
    async showResult() {
        ...
        if (result > 0) {
            ...
            if (selectionList[winIndex].length > 0) {
                const text = selectionList.reduce((str, user) => {
                    return str += ' ' + user.tag;
                }, TEXTLIST[2]);
