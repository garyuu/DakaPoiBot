...
class GuessingGameManager {
    ...
    isRunning(guild) {
        return this.games.hasOwnProperty(guild);
    }

    start(guild) {
        return new Promise((resolve, reject) => {
            try {
                this.games[guild] = new GuessingGame();
                resolve();
            }
            catch(e) {
                reject(e);
    ...
    guess(guild, input) {
        return this.games[guild].guess(input)
                .then(((result) => {
                    if (result.win == true)
                        delete this.games[guild];
                    return result;
                }).bind(this));
    }

    status(guild) {
        return this.games[guild].status();
    }
}
